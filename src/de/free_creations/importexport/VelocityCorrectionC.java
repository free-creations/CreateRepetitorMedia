...
/**
 * Adjusts velocity values. Version C is adapted for the "fluidsysnth"
 * synthesizer. Like Version A, but adds some statistics.
 *
 * @author Harald Postner
 */
public class VelocityCorrectionC {

  private static final double maxRandomRange = 6D;
    ...
    for (int i = 0; i < inputTracks.length; i++) {
      ...
      for (int eventI = 0; eventI < inputTrack.size(); eventI++) {
        ...
        if (trackSet.contains(i)) {          
          ...
          randomVel = randomVelocity(randomVel);
        } else {
  ...
  private static double randomVelocity(double previous) {
