 ...
 */
public class Create_4_WieLieblichSind {
  ...
  final static private String description = "Wie lieblich sind deine Wohnungen";
  final static private String number = "4";
  final static private String camelTitle = "WieLieblichSind";
  final static private int resolution = 480;
  private final File outputMidiFileTemp;

  private Create_4_WieLieblichSind() throws IOException {
    loggingHandler = null;

    orchestraFile = new File("scripts/BrahmsRequiem/resources/orchestra4.mid");
    if (!orchestraFile.exists()) {
      throw new RuntimeException("orchestra4.mid file not found.");
    }
    voicesFile = new File("scripts/BrahmsRequiem/resources/choir4.mid");
    if (voicesFile == null) {
  ...
  private void process() throws InvalidMidiDataException, IOException, JAXBException {
    ...
    // Track 4 - Bassoon
    masterSequence = TrackMerger.process(masterSequence, orchestraSequence, new int[]{8, 9}, 3, "Bassoon", loggingHandler); // 5
    masterSequence = InstrumentExchanger.process(masterSequence, 4, -1, 70, loggingHandler);
    // Track 5 - Horns
    masterSequence = TrackMerger.process(masterSequence, orchestraSequence, new int[]{10}, 4, "Horns", loggingHandler); // 6
    ...
    // Track 14 - Celli
    masterSequence = TrackMerger.process(masterSequence, orchestraSequence, new int[]{18}, 14, "Celli", loggingHandler); // 15
    masterSequence = InstrumentExchanger.process(masterSequence, 14, 48, 42, loggingHandler);
    // Track 15 - Contrabass
    masterSequence = TrackMerger.process(masterSequence, orchestraSequence, new int[]{19}, 15, "Contrabass", loggingHandler); // 16
...
//
    ...
    //MidiSystem.write(voiceSequence, 1, outputMidiFileTemp); //<<<<<<<<<<<<<remove
  ...
  public static void main(String[] args) throws InvalidMidiDataException, IOException, URISyntaxException, JAXBException {
    ...
    Create_4_WieLieblichSind processor = new Create_4_WieLieblichSind();
