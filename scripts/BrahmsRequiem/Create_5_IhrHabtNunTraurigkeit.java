 ...
 */
public class Create_5_IhrHabtNunTraurigkeit {
  ...
  final static private String description = "Ihr habt nun Traurigkeit";
  final static private String number = "5";
  final static private String camelTitle = "IhrHabtNunTraurigkeit";
  final static private int resolution = 480;
  private final File outputMidiFileTemp;

  private Create_5_IhrHabtNunTraurigkeit() throws IOException {
    loggingHandler = null;

    orchestraFile = new File("scripts/BrahmsRequiem/resources/orchestra5.mid");
    if (!orchestraFile.exists()) {
      throw new RuntimeException("orchestra5.mid file not found.");
    }
    voicesFile = new File("scripts/BrahmsRequiem/resources/choir5.mid");
    if (voicesFile == null) {
  ...
  private void process() throws InvalidMidiDataException, IOException, JAXBException {
    ...
    // Track 5 - Horns
    masterSequence = TrackMerger.process(masterSequence, orchestraSequence, new int[]{9}, 4, "Horns", loggingHandler); // 6
    masterSequence = InstrumentExchanger.process(masterSequence, 5, -1, 60, loggingHandler);
    // Track 6 to 9 - empty
    masterSequence.createTrack(); //6
    masterSequence.createTrack(); //7
    masterSequence.createTrack(); //8
    masterSequence.createTrack(); //9
    // Track 10 - Choir
    ...
    // Track 11 - Violins I
    masterSequence = TrackMerger.process(masterSequence, orchestraSequence, new int[]{15}, 11, "Violins I", loggingHandler);// 12
    //masterSequence = InstrumentExchanger.process(masterSequence, 11, 48, 48, loggingHandler);
    // Track 12 violins2
    masterSequence = TrackMerger.process(masterSequence, orchestraSequence, new int[]{16}, 12, "Violins II", loggingHandler); // 13
    masterSequence = InstrumentExchanger.process(masterSequence, 12, 48, 50, loggingHandler);
    // Track 13 - Violas
    masterSequence = TrackMerger.process(masterSequence, orchestraSequence, new int[]{17}, 13, "Violas", loggingHandler); // 14
    masterSequence = InstrumentExchanger.process(masterSequence, 13, 48, 41, loggingHandler);
    // Track 14 - Celli
    masterSequence = TrackMerger.process(masterSequence, orchestraSequence, new int[]{18, 19, 25}, 14, "Celli", loggingHandler); // 15
    masterSequence = InstrumentExchanger.process(masterSequence, 14, 48, 42, loggingHandler);
    // Track 15 - Contrabass
    masterSequence = TrackMerger.process(masterSequence, orchestraSequence, new int[]{20}, 15, "Contrabass", loggingHandler); // 16
...
//
  ...
  public static void main(String[] args) throws InvalidMidiDataException, IOException, URISyntaxException, JAXBException {
    ...
    Create_5_IhrHabtNunTraurigkeit processor = new Create_5_IhrHabtNunTraurigkeit();
