 ...
 */
public class Create_6_DennWirHaben {
  ...
  final static private String description = "Denn wir haben hie keine bleibende Statt";
  final static private String number = "6";
  final static private String camelTitle = "DennWirHaben";
  final static private int resolution = 480;
  private final File outputMidiFileTemp;

  private Create_6_DennWirHaben() throws IOException {
    loggingHandler = null;

    orchestraFile = new File("scripts/BrahmsRequiem/resources/orchestra6.mid");
    if (!orchestraFile.exists()) {
      throw new RuntimeException("orchestra6.mid file not found.");
    }
    voicesFile = new File("scripts/BrahmsRequiem/resources/choir6.mid");
    if (voicesFile == null) {
  ...
  private void process() throws InvalidMidiDataException, IOException, JAXBException {
    ...
    // Track 1 - Flutes
    masterSequence = TrackMerger.process(masterSequence, orchestraSequence, new int[]{5, 6}, 0, "Flute", loggingHandler); // 1
    ...
    // Track 6 - Trumpet
    masterSequence = TrackMerger.process(masterSequence, orchestraSequence, new int[]{12}, 5, "Trumpet", loggingHandler); // 6
    masterSequence = InstrumentExchanger.process(masterSequence, 6, -1, 56, loggingHandler);
    // Track 7 - Trombones
    masterSequence = TrackMerger.process(masterSequence, orchestraSequence, new int[]{13, 14}, 6, "Trombone", loggingHandler); // 6
    masterSequence = InstrumentExchanger.process(masterSequence, 7, -1, 57, loggingHandler);
    // Track 8 - Timpani
    masterSequence = TrackMerger.process(masterSequence, orchestraSequence, new int[]{15}, 7, "Timpani_Viola", loggingHandler); // 6
    masterSequence = InstrumentExchanger.process(masterSequence, 8, 48, 41, loggingHandler);

    // Track 9 - Horns II
    masterSequence = TrackMerger.process(masterSequence, orchestraSequence, new int[]{33}, 8, "Horns II", loggingHandler); // 6
    masterSequence = InstrumentExchanger.process(masterSequence, 9, -1, 60, loggingHandler);

    // Track 10 - Choir
    masterSequence = TrackMerger.process(masterSequence, orchestraSequence, new int[]{16, 17, 18, 19, 20}, 10, "Choir", loggingHandler); // 11
    masterSequence = InstrumentExchanger.process(masterSequence, 10, -1, 19, loggingHandler);
    // Track 11 - Violins I
    masterSequence = TrackMerger.process(masterSequence, orchestraSequence, new int[]{21}, 11, "Violins I", loggingHandler);// 12
    //masterSequence = InstrumentExchanger.process(masterSequence, 11, 48, 48, loggingHandler);
    // Track 12 violins2
    masterSequence = TrackMerger.process(masterSequence, orchestraSequence, new int[]{22}, 12, "Violins II", loggingHandler); // 13
    masterSequence = InstrumentExchanger.process(masterSequence, 12, 48, 50, loggingHandler);
    // Track 13 - Violas
    masterSequence = TrackMerger.process(masterSequence, orchestraSequence, new int[]{23}, 13, "Violas", loggingHandler); // 14
    masterSequence = InstrumentExchanger.process(masterSequence, 13, 48, 41, loggingHandler);
    // Track 14 - Celli
    masterSequence = TrackMerger.process(masterSequence, orchestraSequence, new int[]{24}, 14, "Celli", loggingHandler); // 15
    masterSequence = InstrumentExchanger.process(masterSequence, 14, 48, 42, loggingHandler);
    // Track 15 - Contrabass
    masterSequence = TrackMerger.process(masterSequence, orchestraSequence, new int[]{25}, 15, "Contrabass", loggingHandler); // 16
...
//
    ...
    Sequence voiceSequence = MidiSystem.getSequence(voicesFile);
...
//
//    voiceSequence = TrackMerger.process(voiceSequence, orchestraSequence, new int[]{17}, 0, "SopranVoice", loggingHandler);
//    voiceSequence = TrackMerger.process(voiceSequence, textSequence, new int[]{2}, 0, "SopranText", loggingHandler);
//
//    voiceSequence = TrackMerger.process(voiceSequence, orchestraSequence, new int[]{18}, 0, "AltVoice", loggingHandler);
//    voiceSequence = TrackMerger.process(voiceSequence, textSequence, new int[]{3}, 0, "AltText", loggingHandler);
//
//    voiceSequence = TrackMerger.process(voiceSequence, orchestraSequence, new int[]{19}, 0, "TenorVoice", loggingHandler);
//    voiceSequence = TrackMerger.process(voiceSequence, textSequence, new int[]{4}, 0, "TenorText", loggingHandler);
//
//    voiceSequence = TrackMerger.process(voiceSequence, orchestraSequence, new int[]{20}, 0, "BassVoice", loggingHandler);
//    voiceSequence = TrackMerger.process(voiceSequence, textSequence, new int[]{5}, 0, "BassText", loggingHandler);


    // merge voices into master
    masterSequence = TrackMerger.process(masterSequence, voiceSequence, new int[]{2}, 0, "Sopran", loggingHandler); // 16
    masterSequence = InstrumentExchanger.process(masterSequence, 16, -1, 0, loggingHandler);

    masterSequence = TrackMerger.process(masterSequence, voiceSequence, new int[]{3}, 1, "Alt", loggingHandler); // 17
    masterSequence = InstrumentExchanger.process(masterSequence, 17, -1, 0, loggingHandler);

    masterSequence = TrackMerger.process(masterSequence, voiceSequence, new int[]{4}, 2, "Tenor", loggingHandler); // 18
    masterSequence = InstrumentExchanger.process(masterSequence, 18, -1, 0, loggingHandler);

    masterSequence = TrackMerger.process(masterSequence, voiceSequence, new int[]{5}, 3, "Bass", loggingHandler); // 19
  ...
  public static void main(String[] args) throws InvalidMidiDataException, IOException, URISyntaxException, JAXBException {
    ...
    Create_6_DennWirHaben processor = new Create_6_DennWirHaben();
