...
public class Create_3_Estate {
  ...
  final static private String description = "Le Quattro Stagioni, L'Estate";
  final static private String number = "3";
  final static private String camelTitle = "Estate";

  private Create_3_Estate() throws IOException {
    loggingHandler = null;
    String commonResourcesUrl = "resources/";
    String introUrl = commonResourcesUrl + "Intro3_4.mid";
  ...
  private void process() throws InvalidMidiDataException, IOException, JAXBException {
    ...
    //remove pauses
    long oldPos = findNote(orchestraSequence, 5, eight, 82);
    long newPos = eight;
    orchestraSequence = MidiUtil.leftCut(orchestraSequence, oldPos - newPos);

    // remove pauses
    long toPos = 5 * bar + 3 * eight;
    long fromPos = findNote(orchestraSequence, 5, toPos, 82);
    orchestraSequence = MidiUtil.stretch(orchestraSequence, toPos - eight, fromPos, toPos);

    toPos = 7 * bar + 3 * eight;
    fromPos = findNote(orchestraSequence, 5, toPos, 84);
    orchestraSequence = MidiUtil.stretch(orchestraSequence, toPos - eight, fromPos, toPos);

    toPos = 12 * bar + 2 * quarter;
    fromPos = findNote(orchestraSequence, 5, toPos, 70);
    orchestraSequence = MidiUtil.stretch(orchestraSequence, toPos - eight, fromPos, toPos);

    toPos = 14 * bar + 3 * eight;
    fromPos = findNote(orchestraSequence, 5, toPos, 67);
    orchestraSequence = MidiUtil.stretch(orchestraSequence, toPos - eight / 2, fromPos, toPos);

    toPos = 15 * bar;
    fromPos = 15 * bar + quarter;
    orchestraSequence = MidiUtil.stretch(orchestraSequence, toPos - eight / 2, fromPos, toPos);

    // remove everything afer the 16th bar
    long cutPos = 15 * bar;
...
//    System.out.println("** Sequence length " + masterSequence.getTickLength());
  ...
  public static void main(String[] args) throws InvalidMidiDataException, IOException, URISyntaxException, JAXBException {
    Create_3_Estate processor = new Create_3_Estate();
